...
#define  setLoadServotoPreLoad() \
do{                        \
trigger_servo[2].setAngle(CONFIG_LOAD_SERVO_PRE_LOAD_ANGLE_0); \
trigger_servo[3].setAngle(CONFIG_LOAD_SERVO_PRE_LOAD_ANGLE_0); \
trigger_servo[4].setAngle(CONFIG_LOAD_SERVO_PRE_LOAD_ANGLE_1); \
trigger_servo[5].setAngle(CONFIG_LOAD_SERVO_PRE_LOAD_ANGLE_1); \
}while (0)

#define setLoadServotoLoad() \
do{                        \
trigger_servo[2].setAngle(CONFIG_LOAD_SERVO_LOAD_ANGLE_0); \
trigger_servo[3].setAngle(CONFIG_LOAD_SERVO_LOAD_ANGLE_0); \
trigger_servo[4].setAngle(CONFIG_LOAD_SERVO_LOAD_ANGLE_1); \
trigger_servo[5].setAngle(CONFIG_LOAD_SERVO_LOAD_ANGLE_1); \
}while(0)

#define setLoadServotoLaunch() \
do{                        \
trigger_servo[2].setAngle(CONFIG_LOAD_SERVO_LAUNCH_ANGLE_0); \
trigger_servo[3].setAngle(CONFIG_LOAD_SERVO_LAUNCH_ANGLE_0); \
trigger_servo[4].setAngle(CONFIG_LOAD_SERVO_LAUNCH_ANGLE_1); \
trigger_servo[5].setAngle(CONFIG_LOAD_SERVO_LAUNCH_ANGLE_1); \
}while (0)
...
#define enableLaser() HAL_GPIO_WritePin(GPIOG, GPIO_PIN_13, GPIO_PIN_SET)
    ...
    // 裁判系统判定Flag
    uint8_t last_dart_launch_opening_status_ = E_Gate_State::CLOSED; // 上一次发射状态
    ...
    void FSM::update() {
        ...
        msgDartStatus.motor_yaw_online = motor::MotorYawLS.motor_state_ != motor::E_MotorState::DISCONNECTED;
        ...
        msgDartStatus.motor_loader_angle[0] =
            motor::MotorLoad[0].current_round_ * 8192 + motor::MotorLoad[0].current_angle_;
        msgDartStatus.motor_loader_angle[1] =
            motor::MotorLoad[1].current_round_ * 8192 + motor::MotorLoad[1].current_angle_;
        msgDartStatus.motor_loader_current[0] =
    ...
    public:
        void enter(OpenFSM& fsm) const override {
            ...
            choose_sound_effect(
                randomSongList[HAL_RNG_GetRandomNumber(&hrng) % (sizeof(randomSongList) / sizeof(BuzzerSound))]);
     ...
     */
    template <typename T>
    E_ResetActionReturnState actionResetLSUntilTrigger(motor_controller::pid_angle_velocity_controller<T>& pid,
    ...
    public:
        void enter(OpenFSM& fsm) const override {
            msgDartStatus.dart_state = dart_fsm.openFSM_.
                                                focusEState();
        }
    ...
    public:
        void enter(OpenFSM& fsm) const override {
            ...
            fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State = 0;
            ...
            if (RC_Data.Switch_Left == RC_SW_UP || RC_Data.Switch_Left == RC_SW_MID) {
                ...
                // 扳机锁定在初始位置，不可触发操作，可以操作Yaw、Load电机和扳机丝杆
                {
                    ...
                    if (RC_Data.ch0 > 900 && RC_Data.ch0 < 1100) {
                        ...
                        if (RC_Data.ch0 <= 700 && yaw_switch_state != Triggered)
                            ...
                            motor_controller::MotorYawLSController.target_velocity_ = 0; // 保证不小于零
                        motor_controller::AutoAimController.reset();
                    }
                }
                ...
                int16_t base_velocity = 0;
                {
                    ...
                    // 状态转移
                        ...
                        break;
                    case 1:
                        base_velocity = CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD / 1.6f;
                    // 状态转移
                        if ((RC_Data.ch3 >= 1600) ||
                ...
                case 1:
                    // 将拉簧拉到底
                    // 装填电机向下运动到装填位置
                    base_velocity = CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD_WITH_DART;
                ...
                // 升降机控制
                switch (fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State) {
                case 0:
                    if (RC_Data.ch4_wheel >= 1622 && RC_Data.Switch_Left == RC_SW_MID) {
                        setTriggerServotoTrigger();
                        setLoadServotoPreLoad();
                        fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State = 1;
                    }
                    break;
                case 1:
                    // 装填电机向下运动到装填位置
                    ...
                    if (motor_controller::MotorLoadController[0].current_angle_with_rounds_ >=
                        ...
                        motor_controller::MotorLoadController[1].current_angle_with_rounds_ >=
                        CONFIG_MOTOR_LOAD_ANGLE_DOWN) {
                        fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State = 2;
                ...
                case 2:
                    ...
                    if (xTaskGetTickCount() - fsm.custom<Dart_FSM>()->ActionGeneral_Timer0_ >
                        ...
                        fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State = 3;
                    }
                    break;
                case 3:
                    base_velocity = -CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD_WITH_DART;
                // 装填电机向上运动到发射位置
                    if (motor_controller::MotorLoadController[0].current_angle_with_rounds_ <=
                        ...
                        motor_controller::MotorLoadController[1].current_angle_with_rounds_ <=
                        CONFIG_MOTOR_LOAD_ANGLE_UP) {
                        fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State = 4;
                        setLoadServotoUp();
                        setTriggerServotoReload();
                    }
                    break;
                ...
                if (!fsm.custom<Dart_FSM>()->launch_operating_) {
                    ...
                    if (!fsm.custom<Dart_FSM>()->ActionRemoteandMatch_launch_complete_) {
                        // 如果trigger舵机未到达装填位置
                        if (trigger_servo[0].getAngle() != CONFIG_TRIGGER_SERVO_RELOAD_ANGLE_0 &&
                            trigger_servo[1].getAngle() != CONFIG_TRIGGER_SERVO_RELOAD_ANGLE_1) {
                            setTriggerServotoReload();
                            fsm.custom<Dart_FSM>()->ActionGeneral_Timer1_ = xTaskGetTickCount();
                        } else if (xTaskGetTickCount() - fsm.custom<Dart_FSM>()->ActionGeneral_Timer1_ >
                            pdMS_TO_TICKS(CONFIG_LAUNCH_WAIT_TIME)) {
                            // 判断是否缓动
                            if (motor_controller::MotorLoadController[0].current_angle_with_rounds_ <=
                                CONFIG_MOTOR_LOAD_ANGLE_UP) {
                                base_velocity = -CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD / 6.0f;
                            } else {
                                base_velocity = -CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD;
                            }
    ...
    class ActionMatch_Wait : public OpenFSMAction {
        void enter(OpenFSM& fsm) const override {
            ...
            // if (fsm.custom<Dart_FSM>()->ActionMatch_Wait_Continuous_Fire && game_progress == 4 &&
            //     abs(motor_controller::MotorTriggerLSController.current_angle_with_rounds_ -
            //         motor_controller::MotorTriggerLSController.target_angle_with_rounds_) < 10 &&
            //     abs(motor_controller::MotorYawLSController.current_angle_with_rounds_ -
            //         motor_controller::MotorYawLSController.target_angle_with_rounds_) < 50) {
        ...
        void update(OpenFSM& fsm) const override {
            ...
            state_machine::E_Target_Type target_type = static_cast<E_Target_Type>((dart_info >> 8) & 0x03);
            ...
            // ===== 手动信号域 =====
            // 发射信号四：遥控器信号 外八字；如果在上场模式，则需要在比赛正常进行状态
            ...
            if (((((ext_game_status.stage_remain_time < 4 || dart_remaining_time < 2) && game_progress == 4) ||
            ...
            case 2:
                ...
                // 扳机舵机不触发
                setTriggerServotoTrigger();
#elifndef CONFIG_TRIGGER_SERVO_DEBUG_MODE
                setTriggerServotoReload();
#endif
                if (motor_controller::MotorLoadController[0].current_angle_with_rounds_ >=
                    ...
                    motor_controller::MotorLoadController[1].current_angle_with_rounds_ >=
                    CONFIG_MOTOR_LOAD_ANGLE_POST_LOAD) {
                    base_velocity = -CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD_WITH_DART / 4 * 3;
                } else if (motor_controller::MotorLoadController[0].current_angle_with_rounds_ <=
                    CONFIG_MOTOR_LOAD_ANGLE_UP ||
                    motor_controller::MotorLoadController[1].current_angle_with_rounds_ <=
                    CONFIG_MOTOR_LOAD_ANGLE_UP) {
                    base_velocity = 0;
            ...
            fsm.custom<Dart_FSM>()->ActionMatch_Wait_last_game_progress = game_progress;
    ...
    class ActionMatch_Launch : public OpenFSMAction {
        void enter(OpenFSM& fsm) const override {
            ...
            fsm.custom<Dart_FSM>()->ActionResetMotors_Load_0_Reset_State = 0;
            fsm.custom<Dart_FSM>()->ActionResetMotors_Load_1_Reset_State = 0;
            fsm.custom<Dart_FSM>()->ActionResetMotors_both_initialized = false;
...
#ifndef CONFIG_TRIGGER_SERVO_DEBUG_MODE
    ...
    class ActionMatch_Reload : public OpenFSMAction {
        ...
        void update(OpenFSM& fsm) const override {
            ...
            case 2:
                // 降下升降机并等待时间到达
                if (xTaskGetTickCount() - fsm.custom<Dart_FSM>()->ActionGeneral_Timer0_ >
                    pdMS_TO_TICKS(CONFIG_LIFT_PARTIAL_WAIT_TIME)) {
                    fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State = 3;
                }
                break;
            case 3:
            ...
            // 装填电机向上运动到中间位置
                if (motor_controller::MotorLoadController[0].current_angle_with_rounds_ <=
                    CONFIG_MOTOR_LOAD_ANGLE_POST_RELOAD ||
                    motor_controller::MotorLoadController[1].current_angle_with_rounds_ <=
                    CONFIG_MOTOR_LOAD_ANGLE_POST_RELOAD) {
                    fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State = 4;
                    setLoadServotoUp();
            ...
            case 4:
                ...
                if (xTaskGetTickCount() - fsm.custom<Dart_FSM>()->ActionGeneral_Timer0_ >
                    ...
                    setSlidedownServotoSlide();
                    fsm.custom<Dart_FSM>()->ActionGeneral_Timer1_ = xTaskGetTickCount();
                    fsm.custom<Dart_FSM>()->ActionRemoteandMatch_Reload_State = 5;
                }
                break;
            case 5:
                // 升降机复位后触发下滑，节省时间
